...
=head3 cookie_pairs
  ...
  my $pairs = $cgi->cookie_pairs;

Retrieve request cookies as an array reference of two-element array references.

=head3 cookies

  my $hashref = $cgi->cookies;

Retrieve request cookies as a hash reference. If multiple cookies are passed
with the same name, the value of that name will be an array reference.

=head3 cookie

  my $value = $cgi->cookie('foo');

Retrieve the value of a request cookie by name. If multiple cookies are passed
...
Retrieve values of a request cookie name as an array reference.
