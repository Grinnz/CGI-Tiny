...
=head1 SYNOPSIS
  ...
  cgi {
    ...
    if ($cgi->query_param('download')) {
      $cgi->set_response_attachment('fribble.json');
    }
...
bytes, so non-ASCII filenames make things trickier.
  ...
  cgi {
    ...
    $cgi->set_response_attachment($filepath->basename)->render(file => $filepath);
  };
...
=head3 set_response_attachment

  $cgi = $cgi->set_response_attachment;
  $cgi = $cgi->set_response_attachment($filename);

Sets the response C<Content-Disposition> header to indicate to the client that
the response is a file download, with an optional filename specified in Unicode
characters. No effect after response headers have been rendered.

=head3 set_response_inline

  $cgi = $cgi->set_response_inline;
  $cgi = $cgi->set_response_inline($filename);

Sets the response C<Content-Disposition> header to indicate that the response
should be displayed inline (the default behavior), with an optional filename
specified in Unicode characters. No effect after response headers have been
rendered.
