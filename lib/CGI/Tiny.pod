...
=head3 set_multipart_charset

  $cgi = $cgi->set_multipart_charset('UTF-8');

Sets the default charset for decoding C<multipartE<sol>form-data> text field
values, defaults to C<UTF-8>. Used when the form doesn't have a C<_charset_>
field and the form field doesn't have a charset set in C<Content-Type>.
...
Not available after calling L</"body_params"> or L</"uploads"> (or related
methods) on a C<multipartE<sol>form-data> request, since this type of request
body is not retained in memory after parsing.
...
File uploads are returned as a hash reference containing the following keys:
...
C<Content-Type> header of file field, undef if unspecified.
...
See L</"uploads"> for details on the format of the returned upload.
...
See L</"uploads"> for details on the format of the returned uploads.
...
CGI::Tiny does not have a method-sensitive C<param> accessor; query or body
parameters must be accessed specifically. Uploaded files and their metadata are
accessed with L</"upload"> and do not affect the text parameter accessors.
