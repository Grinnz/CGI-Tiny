...
execution. Just keep in mind that the script will have to be repacked to update
those dependencies.

  $ fatpack pack script.source.cgi > script.cgi

If you need to deploy with JSON support to a Perl older than 5.14:
...
Adds a response C<Set-Cookie> header. No effect after response headers have
been rendered.
