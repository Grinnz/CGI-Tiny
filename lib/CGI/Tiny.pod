...
=head1 SYNOPSIS
  ...
  cgi {
    ...
    if ($cgi->query_param('download')) {
      $cgi->set_response_content_disposition(attachment => 'fribble.json');
    }
...
bytes, so non-ASCII filenames make things trickier.
  ...
  cgi {
    ...
    $cgi->set_response_content_disposition(attachment => $filepath->basename)->render(file => $filepath);
  };
...
=head3 set_response_content_disposition

  $cgi = $cgi->set_response_content_disposition('attachment');
  $cgi = $cgi->set_response_content_disposition(attachment => $filename);
  $cgi = $cgi->set_response_content_disposition('inline'); # default behavior
  $cgi = $cgi->set_response_content_disposition(inline => $filename);
...
characters. C<inline> suggests to display the content inline (the default
behavior), and C<attachment> suggests to download the content as a file. No
