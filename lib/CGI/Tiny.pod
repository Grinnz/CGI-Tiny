...
create and write to the logging target.
  ...
  use Log::Any;
  use Log::Any::Adapter
    {category => 'cgi_script'}, # only log our category here
    File => '/path/to/log/file.log',
    binmode => ':encoding(UTF-8)',
    log_level => $ENV{MY_CGI_LOG_LEVEL} || 'info';

  my $log = Log::Any->get_logger(category => 'cgi_script');
...
=head3 set_response_fixed_length
  ...
  $cgi = $cgi->set_response_fixed_length;
  $cgi = $cgi->set_response_fixed_length(1);

If set to a true value or called without a value before rendering response
headers, a C<Content-Length> header will be set to the size of the encoded
content on the first call to L</"render">, unless it streams the response data
from a C<handle> and thus can't predetermine the content size. L</"render">
cannot be used to provide additional response data if a C<Content-Length>
header was rendered.
